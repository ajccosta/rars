...
Fork of [RARS](https://github.com/rarsm/rars)
...
Documentation for supported [instructions](https://github.com/ajccosta/rars/wiki/Supported-Instructions), [system calls](https://github.com/ajccosta/rars/wiki/Environment-Calls), [assembler directives](https://github.com/ajccosta/rars/wiki/Assembler-Directives) and more can be found on the [wiki](https://github.com/ajccosta/rars/wiki). Documentation included in the download can be accessed via the help menu. 

## Download

RARS is distributed as an executable jar. You will need at least Java 8 to run it. 

The latest stable release can be found [here](https://github.com/ajccosta/rars/releases/latest), a release with the latest developments can be found on the [continuous release](https://github.com/ajccosta/rars/releases/tag/continuous), and the [releases page](https://github.com/ajccosta/rars/releases) contains all previous stable releases with patch notes.

Alternatively, if you wish to make your own jar and/or modify the code, you
should clone the repo with `git clone https://github.com/ajccosta/rars --recursive`.
Running the script `./build-jar.sh` on a Unix system will build `rars.jar`.

## Compiling

Either run `./build-jar.sh` (on Linux) or `gradle build`.
